...
res_login = s.post(url_login, json={'username': 'admin', 'password': 'password123'}, verify=False)
if res_login.status_code != 200 or not res_login.json().get('success'):
    # Try default admin pass
    res_login = s.post(url_login, json={'username': 'admin', 'password': 'password'}, verify=False)
    if not res_login.json().get('success'):
        # Try another default
        res_login = s.post(url_login, json={'username': 'admin', 'password': 'admin1234'}, verify=False)
        print("Login result:", res_login.json())
