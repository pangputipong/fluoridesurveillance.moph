...
@app.route('/api/cases/submit', methods=['POST'])
def submit_case():
    data = request.json
    try:
        with engine.begin() as conn:
            sql = """
            INSERT INTO child_fluorosis_cases 
            (fullname, gender, age, grade, school, province, district, subdistrict, address, years_in_area, survey_date, water_source, water_source_other, tooth_u1, tooth_u2, tooth_u3, tooth_u4, tooth_u5, tooth_u6, tooth_u7, tooth_l1, tooth_l2, tooth_l3, tooth_l4, tooth_l5, tooth_l6, tooth_l7, deans_index, created_by)
            VALUES 
            (:fullname, :gender, :age, :grade, :school, :province, :district, :subdistrict, :address, :years_in_area, :survey_date, :water_source, :water_source_other, :u1, :u2, :u3, :u4, :u5, :u6, :u7, :l1, :l2, :l3, :l4, :l5, :l6, :l7, :deans_index, :created_by)
            """
            conn.execute(text(sql), {
                "fullname": data.get("fullname"), "gender": data.get("gender"), "age": data.get("age"), "grade": data.get("grade"),
                "school": data.get("school"), "province": data.get("province"), "district": data.get("district"),
                "subdistrict": data.get("subdistrict"), "address": data.get("address"), "years_in_area": data.get("years_in_area"),
                "survey_date": data.get("survey_date"), "water_source": data.get("water_source"), "water_source_other": data.get("water_source_other"),
                "u1": data.get("u1"), "u2": data.get("u2"), "u3": data.get("u3"), "u4": data.get("u4"), "u5": data.get("u5"), "u6": data.get("u6"), "u7": data.get("u7"),
                "l1": data.get("l1"), "l2": data.get("l2"), "l3": data.get("l3"), "l4": data.get("l4"), "l5": data.get("l5"), "l6": data.get("l6"), "l7": data.get("l7"),
                "deans_index": data.get("deans_index"), "created_by": data.get("created_by")
            })
